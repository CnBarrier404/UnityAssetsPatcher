...
public sealed class WorkflowServiceCompatibilityTests
{
    ...
    public void PreviewUninstall_WhenFilesAndInstallLayersConflict_PreservesBlockersIntegrityAndCanUninstall()
    {
        ...
        TestDependencies.WriteCommittedRecord(repository, target, repository.GetInstallDirectory(target.Id));
        TestDependencies.WriteCommittedRecord(repository, blocker, repository.GetInstallDirectory(blocker.Id));
        ...
        StubAssetsFileService assetsFileService)
    {
        ...
        services.AddLogging();
        services.AddUnityAssetsPatcherInfrastructure(() => throw new InvalidOperationException(
            "The test asset service replaces AssetsTools.NET."));
    ...
    {
        var fieldTree = TestAssetField.Create("Camera", "Camera", null,
        [
