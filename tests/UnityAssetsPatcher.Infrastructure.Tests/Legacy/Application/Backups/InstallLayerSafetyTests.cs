...
public sealed class InstallLayerSafetyTests
{
    ...
    public void InstallSequenceAllocator_AllocatesPerGameAndRejectsDuplicateSequence()
    {
        ...
        Assert.Equal(2, InstallSequenceAllocator.Allocate([first, otherGame], "game-a", "repository"));
        Assert.Throws<InvalidDataException>(() =>
            InstallSequenceAllocator.Allocate(
                [first, Record("duplicate", "game-a", 1, "a.assets")],
                "game-a",
                "repository"));
    }
    ...
    public void InstallRecordValidator_RejectsWrongRepositoryOrInvalidIdentity(
        ...
        long sequence)
    {
        InstallRecord source = Record("a", fingerprint, sequence, "a.assets");
        InstallRecord record = CopyRecord(source, repositoryId: repositoryId);
    ...
    {
        Assert.ThrowsAny<ArgumentException>(() => new FileIntegrity(length, sha256));
    }
    ...
    private static InstallRecord CopyRecord(InstallRecord source, string? repositoryId = null)
    {
        return new InstallRecord(
            repositoryId ?? source.RepositoryId,
            source.GameInstanceFingerprint,
            source.InstallSequence,
            source.Id,
            source.InstalledAt,
            source.ModName,
            source.ModVersion,
    ...
    private sealed class Scenario : IDisposable
    {
        ...
        public static Scenario Create()
        {
            ...
            TestDependencies.WriteCommittedRecord(store, CreateScenarioRecord(
